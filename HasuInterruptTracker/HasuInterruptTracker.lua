--[[
    Hasu's Interrupt Tracker - Midnight 12.0.x

    Author: Hasu (base by Loxxar)

    - Addon-to-addon sync (SendAddonMessage)
    - ElvUI auto-detection (font, texture)
    - Simplified config (/hasu)
...
local ADDON_NAME = "HasuInterruptTracker"
local MSG_PREFIX = "LOXX"
local HASU_VERSION  = "2.0.0.0"
local HASU_DB_VERSION = 4 -- bump when SavedVars schema changes
local L = HasuL or {}     -- localization table (set by localization.lua)
...
local DEFAULTS = {
    ...
    ccFrameY          = nil,
    ccHiddenClasses   = {},
    ccSpellState      = {},   -- [specID][spellID]=true(force show)/false(force hide)/nil(default)
    ccBarHeight       = 22,
    ccFrameWidth      = 200,
    ccAlpha           = 0.9,
    ...
    showEstimatedIndicator = true,   -- show ~ badge on bars without Hasu addon
    enableBroadcast        = true,   -- broadcast own kick/CC data to group (HASU protocol)
}
...
-- ── Hasu: interrupt spell set (hidden by default in CC window) ────────────────
-- Independent of HasuCC_Data.lua — always available.
local HASU_INTERRUPT_SPELL_SET = {
    ...
    [78675]=true,  -- Solar Beam         (Balance Druid)
}

-- ── Hasu: spells that need a talent to have CC utility ────────────────────────
-- [castSpellID] = required talent spellID
local HASU_REQUIRE_TALENT_CASTS = {
    [43265] = 273952,  -- Death and Decay: Poigne des morts / Grip of the Dead
    [703]   = 196861,  -- Garrote:         Fil de fer / Wire Garotte (silence)
}

local ccAddonUsers         = {}   -- CC Tracker: name → { class, specID, isSelf, hasAddon, activeTalents={}, ccs={[spellID]={name,baseCd,cdEnd,maxCharges,icon}}, ccOrder={} }
...
local StartNewRun          -- defined in the stats block; used by /hasu reset
...
-- Error logger
------------------------------------------------------------
local function HasuLogError(msg)
    local ts = date("%H:%M:%S")
    if msg == hasuLastErr then
        hasuErrCount = hasuErrCount + 1
        if hasuErrCount > 5 then return end -- suppress storm of identical errors
        msg = msg .. " (x" .. hasuErrCount .. ")"
    else
        hasuLastErr  = msg
        hasuErrCount = 1
    end
    local entry = "[" .. ts .. "] " .. msg
    table.insert(hasuErrorLog, 1, entry) -- newest first
    while #hasuErrorLog > 50 do table.remove(hasuErrorLog) end
    if HasuSavedVars then HasuSavedVars.hasuErrorLog = hasuErrorLog end
end
...
------------------------------------------------------------
-- Dungeon logger — /hasu record  (independent of spy mode)
------------------------------------------------------------
local function DLog(cat, msg)
    if not hasuDungeonLogActive then return end
    ...
    if ok and type(entry) == "string" then
        table.insert(hasuDungeonLog, entry)
        if #hasuDungeonLog > DUNGEON_LOG_MAX then
            table.remove(hasuDungeonLog, 1)
        end
    end
end
...
-- Communication
------------------------------------------------------------
local function SendHasu(msg)
    -- Don't send when solo: PARTY/INSTANCE_CHAT would print "You are not in a group."
    if not IsInGroup() then return end
    -- Respect user's broadcast preference (can be disabled in /hasu config).
    ...
    if not ok then
        ...
        DLog("COMM", "SendHasu FAILED ch=" .. channel .. " msg=" .. tostring(msg) .. " err=" .. errStr)
        HasuLogError("SendHasu failed: " .. errStr)
    end
end
...
local function AnnounceJoin()
    ...
    local joinMsg = "JOIN:" .. myClass .. ":" .. mySpellID .. ":" .. cd .. ":" .. HASU_VERSION .. ":" .. specID
    SendHasu(joinMsg)
    DLog("JOIN", "SENT cls=" .. tostring(myClass) .. " spellID=" .. tostring(mySpellID) .. " cd=" .. cd .. " ver=" .. HASU_VERSION .. " spec=" .. specID)
    if HasuRotation then HasuRotation.UpdateRoster(partyAddonUsers, myName) end
end
...
local function AnnounceState()
    ...
    if remaining > 0 then
        ...
        SendHasu("STATE:" .. string.format("%.1f", remaining))
    end
end
...
local function OnAddonMessage(prefix, message, channel, sender)
    ...
    if command == "PING" then
        ...
        print("|cFF00DDDD[HASU]|r Received PING from |cFF00FF00" ..
    ...
    elseif command == "CAST" then
        ...
        elseif cd and cd > 0 then
            DLog("CAST", "CAST from " .. shortName .. " not in partyAddonUsers — sending HELLO to request JOIN")
            -- Kick received before JOIN was processed; ask peer to re-announce
            SendHasu("HELLO")
        else
    ...
    elseif command == "CCCAST" then
        ...
        if spellID and cd then
            ...
            local legacyEntry = (not ccEntry) and CC_SPELLS[spellID]
            local ccName  = (ccEntry and ccEntry.name)  or (legacyEntry and legacyEntry.name)  or "?"
            local ccClass = (ccEntry and ccEntry.class) or (legacyEntry and legacyEntry.class) or nil

            -- Ensure the sender has an entry in ccAddonUsers
            ...
            if ccAddonUsers[shortName] then
                ...
                local ccs   = ccAddonUsers[shortName].ccs
                local order = ccAddonUsers[shortName].ccOrder
...
local function OnSpellCastSucceeded(unit, castGUID, spellID, isParty, cleanName)
    ...
    SendHasu(castMsg)
    C_Timer.After(0.05, function() SendHasu(castMsg) end)
    C_Timer.After(0.10, function() SendHasu(castMsg) end)
    RecordKick(myName)
end
            ...
            if entryID then
                ...
                if ok4 and entryInfo and entryInfo.definitionID then
                    ...
                    if ok5 and defInfo and defInfo.spellID then
                        ...
                        -- Hasu: record talent as active (numeric + string key for 12.0 compat)
                        local ok_at = pcall(function() activeTalents[defSpellID] = true end)
                        if ok_at and defSpellStr then activeTalents[defSpellStr] = true end
    ...
    if HasuCCData and specID and specID > 0 then
        ...
        if ccList then
            if not ccAddonUsers[name] then
                ccAddonUsers[name] = {
                    class         = (info and info.class) or (select(2,UnitClass(unit))) or "UNKNOWN",
                    specID        = specID,
                    hasAddon      = false,
                ...
                ccAddonUsers[name].specID        = specID
                ccAddonUsers[name].activeTalents = activeTalents
                ccAddonUsers[name].ccs           = ccAddonUsers[name].ccs   or {}
                ccAddonUsers[name].ccOrder       = {}
            end
            ...
            for _, entry in ipairs(ccList) do
                ...
                if allowed then
                    ...
                    if entry.extraChargeTalent then
                        if activeTalents[entry.extraChargeTalent]
                            or (activeTalents[tostring(entry.extraChargeTalent)] == true) then
                            maxCharges = 2
                        end
                    end
...
local function RebuildBars()
    ...
    for i = 1, currentMaxBars do
        ...
        -- Estimated indicator (~ bottom-right, yellow-gray — no Hasu addon on this player)
        ...
        f:SetScript("OnEnter", function(self)
            ...
            -- Indicate when CD tracking is estimated (no Hasu addon on this player).
...
local function UpdateDisplay()
    ...
    local function RenderPartyBar(bar, icon, name, col, baseCd, rem, spellName, isEstimated)
        ...
        if bar.missedBadge then
            local misses = hasuCurrentRun and hasuCurrentRun.missedKicks and hasuCurrentRun.missedKicks[name]
    ...
    if mySpellData then
        ...
        if bar.missedBadge then
            local misses = hasuCurrentRun and hasuCurrentRun.missedKicks and myName and hasuCurrentRun.missedKicks[myName]
            if misses and misses > 0 then
                bar.missedBadge:SetText("x" .. misses)
                bar.missedBadge:Show()
            else
        ...
        for name, info in pairs(partyAddonUsers) do
            ...
            if ok and data then
                ...
                table.insert(cachedPartyEntries, {
                    ...
                    -- (no Hasu addon message received) — CD tracking is approximate.
                    isEstimated = (info.addonVersion == nil),
                })
            elseif spyMode and info.spellID then
                print("|cFFFF4400[HASU]|r Unknown spellID=" .. tostring(info.spellID) .. " for " .. name)
            end
    ...
    if HasuRotation then
        HasuRotation.MarkNextKicker(
            bars, barIdx - 1,
...
local function HasuSaveFramePosition(frame)
    if not frame then return false end
    local x, y = frame:GetLeft(), frame:GetTop()
    if not x or not y then return false end
    x, y = math.floor(x + 0.5), math.floor(y + 0.5)
    HasuAccountVars = HasuAccountVars or {}
    HasuAccountVars.frameX = x
    HasuAccountVars.frameY = y
    if db then
        db.frameX = x
        db.frameY = y
    end
...
local function HasuRestoreFramePosition(frame)
    if not frame then return end
    local x, y
    if db and db.frameX and db.frameY then
        x, y = db.frameX, db.frameY
    elseif HasuAccountVars and HasuAccountVars.frameX and HasuAccountVars.frameY then
        x, y = HasuAccountVars.frameX, HasuAccountVars.frameY
    end
    if x and y then
        frame:ClearAllPoints()
        frame:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT", x, y)
...
local function CreateConfigPanel()
    ...
    configFrame = CreateFrame("Frame", "HasuConfigFrame", UIParent, "BasicFrameTemplate")
    ...
    table.insert(UISpecialFrames, "HasuConfigFrame")
    ...
    hdrVersion:SetText("|cFFAA8800v" .. HASU_VERSION .. "|r")
    ...
    local alphaSlider = MakeSlider("Hasu_Slider_alpha", configFrame)
    ...
    local widthSlider = MakeSlider("Hasu_Slider_width", configFrame)
    ...
    local heightSlider = MakeSlider("Hasu_Slider_height", configFrame)
    ...
    local nameSlider = MakeSlider("Hasu_Slider_nameFont", configFrame)
    ...
    local cdSlider = MakeSlider("Hasu_Slider_cdFont", configFrame)
    ...
    local readySlider = MakeSlider("Hasu_Slider_readyFont", configFrame)
    ...
    ccConfigBtn:SetSize(130, 22)
    ccConfigBtn:SetPoint("TOPLEFT", R_CBX1, yR)
    ccConfigBtn:SetText("Masquer classes CC")
    ccConfigBtn:SetScript("OnClick", function()
        if ShowCCConfig then ShowCCConfig() end
    end)
    -- Hasu: second button — per-spec CC spell visibility
    local ccSpellsBtn = CreateFrame("Button", nil, configFrame, "UIPanelButtonTemplate")
    ccSpellsBtn:SetSize(130, 22)
    ccSpellsBtn:SetPoint("TOPLEFT", ccConfigBtn, "TOPRIGHT", 4, 0)
    ccSpellsBtn:SetText("Sorts CC par spec")
    ccSpellsBtn:SetScript("OnClick", function()
        if ShowCCSpellConfig then ShowCCSpellConfig() end
    end)

    -- CC bar: Opacity
    yR = yR - 48
    local initCCAlpha = db.ccAlpha or 0.9
    local ccAlphaSlider = MakeSlider("Hasu_CfgCC_Slider_alpha", configFrame)
    ...
    local ccWidthSlider2 = MakeSlider("Hasu_CfgCC_Slider_width", configFrame)
    ...
    local ccHeightSlider2 = MakeSlider("Hasu_CfgCC_Slider_height", configFrame)
    ...
    local ccNameFontSlider2 = MakeSlider("Hasu_CfgCC_Slider_nameFont", configFrame)
    ...
    local ccCdFontSlider2 = MakeSlider("Hasu_CfgCC_Slider_cdFont", configFrame)
    ...
    savePosBtn:SetScript("OnClick", function()
        ...
        local ok, err = pcall(function()
            if not mainFrame then
                toChat("|cFF00DDDD[HASU]|r " .. L["POS_NO_FRAME"])
                return
            end
            if HasuSaveFramePosition(mainFrame) then
                toChat("|cFF00DDDD[HASU]|r |cFF44FF44" .. L["POS_SAVED"] .. "|r")
                toCenter("HASU: " .. L["POS_SAVED"])
            else
                toChat("|cFF00DDDD[HASU]|r " .. L["POS_HIDDEN"])
            end
        end)
        if not ok and err then
            toChat("|cFFFF4444[HASU]|r Error: " .. tostring(err))
        end
    end)
    ...
    commandsBtn:SetScript("OnClick", function()
        local p = "|cFF00DDDD[HASU]|r "
        print(p .. "|cFFFFFF00/hasu|r — toggle tracker")
        print(p .. "|cFFFFFF00/hasu score|r — all-time kick score")
        print(p .. "|cFFFFFF00/hasu runs|r — show run history")
        print(p .. "|cFFFFFF00/hasu csv|r — export stats as CSV")
        print(p .. "|cFFFFFF00/hasu cc|r — toggle CC Tracker")
        print(p .. "|cFFFFFF00/hasu reset|r — reset current run")
        print(p .. "|cFFFFFF00/hasu config|r — open settings")
    end)
    ...
    footerVer:SetText("|cFF888888v" .. HASU_VERSION .. "|r")
...
local function CreateUI()
    mainFrame = CreateFrame("Frame", "HasuMainFrame", UIParent)
    mainFrame:SetSize(db.frameWidth, 200)
    -- Restore saved position
    HasuRestoreFramePosition(mainFrame)
    ...
    C_Timer.After(0, function()
        if mainFrame and not (HasuAccountVars and HasuAccountVars.frameX and HasuAccountVars.frameY)
            and not (db and db.frameX and db.frameY) then
            HasuSaveFramePosition(mainFrame)
        end
    end)
    DetectElvUI()
    RebuildBars()
...
------------------------------------------------------------
-- All-time stats aggregator (used by /hasu csv and /hasu score)
------------------------------------------------------------
local function ComputeAllStats()
    local data = {}
    local allRuns = {}
    if hasuCurrentRun and next(hasuCurrentRun.players) then
        allRuns[#allRuns + 1] = hasuCurrentRun
    end
    for _, run in ipairs(HasuSavedVars.loxxRunHistory or {}) do
...
local function SetupSlash()
    SLASH_HASU1 = "/hasu"
    SlashCmdList["HASU"] = function(msg)
        ...
        elseif cmd == "config" or cmd == "options" or cmd == "settings" then
            CreateConfigPanel()
        elseif cmd == "cc" or cmd == "ccspells" then
            if ShowCCSpellConfig then ShowCCSpellConfig() end
        elseif cmd == "lock" then
            db.locked = true
            print("|cFF00DDDD[HASU]|r " .. L["CMD_LOCKED"])
        elseif cmd == "unlock" then
            db.locked = false
            print("|cFF00DDDD[HASU]|r " .. L["CMD_UNLOCKED"])
        elseif cmd == "test" then
            if testMode then
                ...
                if hasuCurrentRun and hasuCurrentRun.instanceID == -1 then
                    hasuCurrentRun = nil
                end
                SetDisplayDirty()
                print("|cFF00DDDD[HASU]|r " .. L["CMD_TEST_OFF"])
            else
                ...
                testTicker = C_Timer.NewTicker(2, function()
                    ...
                    for name, info in pairs(partyAddonUsers) do
                        ...
                        if info.cdEnd < now and math.random() < 0.3 then
                            ...
                            if hasuCurrentRun then
                                hasuCurrentRun.players[name] = (hasuCurrentRun.players[name] or 0) + 1
                            end
                        end
                    end
                end)
                print("|cFF00DDDD[HASU]|r " .. L["CMD_TEST_ON"])
            end
        elseif cmd == "ping" then
            print("|cFF00DDDD[HASU]|r === PING ===")
        ...
        elseif cmd == "spy" then
            if spyMode then
                spyMode = false
                print("|cFF00DDDD[HASU]|r Spy mode |cFFFF4444OFF|r")
            else
                spyMode = true
                spyCastCount = 0
                print("|cFF00DDDD[HASU]|r Spy mode |cFF00FF00ON|r")
        ...
        elseif cmd == "pos" then
            ...
            if mainFrame then
                ...
                print("|cFF00DDDD[HASU]|r Current position: " .. tostring(l) .. ", " .. tostring(t))
                print("  Account saved: " ..
                    tostring(HasuAccountVars and HasuAccountVars.frameX) ..
                    ", " .. tostring(HasuAccountVars and HasuAccountVars.frameY))
                print("  Char saved: " .. tostring(db and db.frameX) .. ", " .. tostring(db and db.frameY))
            else
                print("|cFF00DDDD[HASU]|r mainFrame not yet created.")
            end
        elseif cmd == "debug" then
            print("|cFF00DDDD[HASU]|r v" ..
                HASU_VERSION .. " | " .. tostring(myClass) .. " | CD cached: " .. tostring(myCachedCD))
            for name, info in pairs(partyAddonUsers) do
        ...
        elseif cmd == "ccdiag" or cmd == "cc-diag" or cmd == "ccdebug" then
            -- Verbose CC / party diagnostics: dumps every tracked user and the
            -- state of inspect, addon-message flow and CC ability lists.
            print("|cFF00DDDD[HASU]|r === CC DIAG ===")
            print(string.format("  Self: %s class=%s spec=%s prefix=%s",
                tostring(myName), tostring(myClass),
                tostring(GetSpecialization and GetSpecialization() or "?"),
                tostring(MSG_PREFIX)))
            print(string.format("  IsInGroup=%s IsInInstance=%s",
                tostring(IsInGroup()), tostring((IsInInstance()))))
            -- partyAddonUsers (interrupt side)
            local pCount = 0
            for _ in pairs(partyAddonUsers) do pCount = pCount + 1 end
            print(string.format("  partyAddonUsers (interrupt side): %d", pCount))
            for name, info in pairs(partyAddonUsers) do
                print(string.format("    - %s cls=%s spellID=%s baseCd=%s ver=%s spec=%s",
                    name, tostring(info.class), tostring(info.spellID),
                    tostring(info.baseCd), tostring(info.addonVersion),
                    tostring(info.specID)))
            end
            -- ccAddonUsers (CC tracker side)
            local cCount = 0
            for _ in pairs(ccAddonUsers) do cCount = cCount + 1 end
            print(string.format("  ccAddonUsers (CC tracker): %d", cCount))
            for name, info in pairs(ccAddonUsers) do
                local n = 0
                for _ in pairs(info.ccs or {}) do n = n + 1 end
                print(string.format("    - %s cls=%s spec=%s isSelf=%s hasAddon=%s ccCount=%d",
                    name, tostring(info.class), tostring(info.specID),
                    tostring(info.isSelf), tostring(info.hasAddon), n))
                local now = GetTime()
                for sid, cc in pairs(info.ccs or {}) do
                    local rem = (cc.cdEnd and cc.cdEnd > now) and (cc.cdEnd - now) or 0
                    local isInt = HASU_INTERRUPT_SPELL_SET[sid] and " [INTERRUPT]" or ""
                    print(string.format("        %s  spellID=%d  baseCd=%s  charges=%s  rem=%.1f%s",
                        tostring(cc.name), sid, tostring(cc.baseCd),
                        tostring(cc.maxCharges), rem, isInt))
                end
            end
        ...
        elseif cmd == "stats clear" then
            HasuSavedVars.loxxRunHistory = {}
            if hasuCurrentRun and hasuCurrentRun.instanceID ~= -1 then hasuCurrentRun = nil end
            if statsFrame then
                statsFrame:Hide(); statsFrame = nil
            end
            print("|cFF00DDDD[HASU]|r " .. L["CMD_HIST_CLEAR"])
        elseif cmd == "record" then
            if hasuDungeonLogActive then
                hasuDungeonLogActive = false
                print("|cFF00DDDD[HASU]|r Dungeon log |cFFFF4444STOPPED|r — " ..
                    #hasuDungeonLog .. " entries. /hasu record show to view.")
            else
                hasuDungeonLog = {}
                hasuDungeonLogActive = true
                ...
                print(
                    "|cFF00DDDD[HASU]|r Dungeon log |cFF00FF00STARTED|r. /hasu record to stop, /hasu record show to view.")
            end
        elseif cmd == "record show" then
            ...
            if #hasuDungeonLog == 0 then
                print("|cFF00DDDD[HASU]|r No dungeon log entries yet.")
            else
                ...
                local total = #hasuDungeonLog
                title:SetText("|cFF00DDDDHasu Dungeon Log|r — " ..
                    total .. " entries" .. (hasuDungeonLogActive and " |cFF00FF00[LIVE]|r" or ""))
                ...
                local function RebuildLogText(filter)
                    ...
                    title:SetText("|cFF00DDDDHasu Dungeon Log|r [" .. label .. "] — "
                        .. shown .. "/" .. count .. " of " .. total
                        .. (hasuDungeonLogActive and " |cFF00FF00[LIVE]|r" or ""))
                end
        ...
        elseif cmd == "record clear" then
            ...
            print("|cFF00DDDD[HASU]|r Dungeon log cleared.")
        elseif cmd == "logs" or cmd == "log" then
            if #hasuErrorLog == 0 then
                print("|cFF00DDDD[HASU]|r " .. L["CMD_NO_LOGS"])
            else
                print("|cFF00DDDD[HASU]|r === Recent errors (" .. #hasuErrorLog .. ") ===")
                for i = 1, math.min(20, #hasuErrorLog) do
                    print("|cFFFF4444[" .. i .. "]|r " .. hasuErrorLog[i])
                end
                if #hasuErrorLog > 20 then
                    print("  ... " .. (#hasuErrorLog - 20) .. " more. Use /hasu logs clear to wipe.")
                end
            end
        elseif cmd == "logs clear" or cmd == "log clear" then
            hasuErrorLog = {}
            if HasuSavedVars then HasuSavedVars.hasuErrorLog = {} end
            print("|cFF00DDDD[HASU]|r " .. L["CMD_LOG_CLEAR"])
        elseif cmd == "miss" then
            if not hasuCurrentRun then
                print("|cFF00DDDD[HASU]|r No active run.")
            elseif not hasuCurrentRun.missedKicks or not next(hasuCurrentRun.missedKicks) then
                print("|cFF00DDDD[HASU]|r No missed kicks this run.")
            else
                print("|cFF00DDDD[HASU]|r Missed kicks — " .. (hasuCurrentRun.dungeon or "?") .. ":")
                for name, count in pairs(hasuCurrentRun.missedKicks) do
        ...
        elseif cmd == "export" then
            if not hasuCurrentRun then
                ...
                local r = hasuCurrentRun
                local lvl = (r.keyLevel and r.keyLevel > 0) and (" +" .. r.keyLevel) or ""
                print("|cFFFFD100[Hasu Export]|r " .. (r.dungeon or "?") .. lvl .. " — " .. (r.date or "?"))
        ...
        elseif cmd == "reset" then
            StartNewRun()
            displayDirty = true
            print("|cFF00DDDD[HASU]|r Run reset.")
        elseif cmd == "score" then
            ShowScoreWindow()
        elseif cmd == "csv" then
            local stats = ComputeAllStats()
            if #stats == 0 then
                print("|cFF00DDDD[HASU]|r No data yet.")
            else
                print("|cFF00DDDD[HASU]|r All-time stats (CSV):")
...
local function ArchiveCurrentRun()
    if not (hasuCurrentRun and next(hasuCurrentRun.players)) then return end
    if hasuCurrentRun.startTime then
        hasuCurrentRun.duration = math.floor(GetTime() - hasuCurrentRun.startTime)
        hasuCurrentRun.startTime = nil -- don't persist the raw timestamp
    end
    HasuSavedVars.loxxRunHistory = HasuSavedVars.loxxRunHistory or {}
    -- Avoid double-archiving the same run (instanceID + date match)
    local top = HasuSavedVars.loxxRunHistory[1]
    if top and top.instanceID == hasuCurrentRun.instanceID and top.date == hasuCurrentRun.date then
        return
    end
    table.insert(HasuSavedVars.loxxRunHistory, 1, hasuCurrentRun)
    local maxH = (db and db.maxRunHistory) or 50
    while #HasuSavedVars.loxxRunHistory > maxH do
        table.remove(HasuSavedVars.loxxRunHistory)
    end
end
...
RecordKick = function(playerName)
    if not hasuCurrentRun then return end
    if not IsInInstance() then return end
    hasuCurrentRun.players[playerName] = (hasuCurrentRun.players[playerName] or 0) + 1
end

RecordMissedKick = function(playerName)
    if not hasuCurrentRun then return end
    if not IsInInstance() then return end
    hasuCurrentRun.missedKicks = hasuCurrentRun.missedKicks or {}
    hasuCurrentRun.missedKicks[playerName] = (hasuCurrentRun.missedKicks[playerName] or 0) + 1
...
ShowStatsWindow = function(charFilterOverride)
    ...
    local function ComputeTotals(charFilter)
        ...
        local runCount = 0
        if hasuCurrentRun and next(hasuCurrentRun.players) then
            if not charFilter or hasuCurrentRun.character == charFilter then
                runCount = runCount + 1
                for name, kicks in pairs(hasuCurrentRun.players) do
    ...
    -- ── Frame ────────────────────────────────────────────────────
    local sf = CreateFrame("Frame", "HasuStatsFrame", UIParent, "BasicFrameTemplate")
    sf:SetSize(SW, SH)
    ...
    if hasuCurrentRun then
        ...
        local keyStr = (hasuCurrentRun.keyLevel or 0) > 0 and (" [+" .. hasuCurrentRun.keyLevel .. "]") or ""
        AddLine(
            "|cFFFFD100" ..
            (hasuCurrentRun.dungeon or "?") .. keyStr .. "|r  |cFF888888" .. (hasuCurrentRun.date or "") .. "|r", 0)
        y = y - 16
        if hasuCurrentRun.players and next(hasuCurrentRun.players) then
            for _, row in ipairs(SortedPlayers(hasuCurrentRun.players)) do
                AddLine(
                    "  " ..
                    row.name .. " — " .. row.kicks .. " " .. (row.kicks ~= 1 and L["STATS_KICKS"] or L["STATS_KICK"]),
                    8)
    ...
    -- ── Section Historique ───────────────────────────────────────
    local allRuns = HasuSavedVars.loxxRunHistory or {}
    local runs = {}
    ...
    if #runs > 0 then
        ...
        for i, run in ipairs(runs) do
            ...
            delBtn:SetScript("OnClick", function()
                table.remove(HasuSavedVars.loxxRunHistory, origIdx)
...
ShowScoreWindow = function()
    ...
    local sf = CreateFrame("Frame", "HasuScoreFrame", UIParent, "BasicFrameTemplate")
    sf:SetSize(SW, SH)
    ...
    clearBtn:SetScript("OnClick", function()
        ...
        sf:Hide()
        scoreFrame = nil
        print("|cFF00DDDD[HASU]|r All-time score cleared.")
    end)
...
local function RegisterBlizzardOptions()
    ...
    panel.name  = "Hasu's Interrupt Tracker"
    ...
    local function BC(label, dbKey, x, y)
        local cb = CreateFrame("CheckButton", "Hasu_Blizz_" .. dbKey, panel, "UICheckButtonTemplate")
        cb:SetPoint("TOPLEFT", x, y)
    ...
    title:SetText("|cFF00DDDDHasu's Interrupt Tracker|r")
    ...
    BS("Hasu_Blizz_NameFont", panel, LSL, yL, 2, 32, 1,
    ...
    yL = yL - 44
    BS("Hasu_Blizz_CdFont", panel, LSL, yL, 2, 32, 1,
    ...
    yL = yL - 44
    BS("Hasu_Blizz_ReadyFont", panel, LSL, yL, 2, 32, 1,
    ...
    BS("Hasu_Blizz_Width", panel, RSL, yR, 120, 400, 10,
    ...
    yR = yR - 44
    BS("Hasu_Blizz_Height", panel, RSL, yR, 14, 50, 1,
        db.barHeight or 20,
    ...
    BS("Hasu_Blizz_Alpha", panel, RSL, yR, 0.3, 1.0, 0.05,
        db.alpha or 0.9,
    ...
    if Settings and Settings.RegisterAddOnCategory then
        ...
        category.ID = "HasuInterruptTracker"
...
local function Initialize()
    HasuSavedVars = HasuSavedVars or {}
    HasuSavedVars.db = HasuSavedVars.db or {}
    db = HasuSavedVars.db

    -- Migration: move legacy root-level data into db table
    for k, v in pairs(DEFAULTS) do
        if HasuSavedVars[k] ~= nil and db[k] == nil then
            db[k] = HasuSavedVars[k]
            HasuSavedVars[k] = nil
        end
    end
    ...
    if savedVer < HASU_DB_VERSION then
    ...
    db.dbVersion                 = HASU_DB_VERSION

    -- Persistent storage outside db (not subject to DEFAULTS migrations)
    HasuSavedVars.loxxRunHistory = HasuSavedVars.loxxRunHistory or {}
    HasuSavedVars.hasuErrorLog   = HasuSavedVars.hasuErrorLog or {}
    hasuErrorLog                 = HasuSavedVars.hasuErrorLog
    ...
    print("|cFF00DDDD[Hasu's Interrupt Tracker]|r v" .. HASU_VERSION .. " | /hasu")
end
    ...
    if unit == "player" then
        ...
        if hasuCC or legacyCC then
            ...
            if myName and ccAddonUsers[myName] and ccAddonUsers[myName].ccs then
                ...
                local _reqTalent = HASU_REQUIRE_TALENT_CASTS[spellID]
                local _hasTalent = true
                if _reqTalent then
                    local _ok1, _r1 = pcall(IsPlayerSpell, _reqTalent)
                    if _ok1 and _r1 then
                        _hasTalent = true
                    else
                        local _ok2, _r2 = pcall(IsSpellKnown, _reqTalent)
                        _hasTalent = (_ok2 and _r2) and true or false
                    end
                end

                if _hasTalent then
                    local ccs   = ccAddonUsers[myName].ccs
                    local order = ccAddonUsers[myName].ccOrder or {}
                    if not ccs[spellID] then
        ...
        if data then
            ...
            if not isExtra then
                ...
                if mySpellID and usedID ~= mySpellID then
                    ...
                    SendHasu(petCastMsg)
                    C_Timer.After(0.05, function() SendHasu(petCastMsg) end)
                    C_Timer.After(0.10, function() SendHasu(petCastMsg) end)
                    RecordKick(myName)
                    if spyMode then
...
ef:SetScript("OnEvent", function(_, event, arg1, arg2, arg3, arg4)
    ...
    elseif event == "INSPECT_READY" then
        if inspectBusy and inspectUnit then
            ...
            if ok then
                ...
                HasuLogError("INSPECT_READY scan failed for " .. targetName .. ": " .. tostring(err))
                if spyMode then
    ...
    elseif event == "PLAYER_SPECIALIZATION_CHANGED" then
        ...
        elseif changedUnit == "player" then
            DLog("SPEC", "SELF spec changed → re-detecting interrupt + CC")
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        if inInst and (instType == "party" or instType == "raid" or instType == "arena") then
            ...
            if not hasuCurrentRun or hasuCurrentRun.instanceID ~= newInstanceID then
    ...
    elseif event == "CHALLENGE_MODE_START" then
        ...
        print("|cFF00DDDD[HASU]|r Dungeon log started. /hasu record show to view.")
    ...
    elseif event == "PLAYER_LOGOUT" then
        ArchiveCurrentRun()
        if mainFrame then HasuSaveFramePosition(mainFrame) end
    end
end)
...
-- Hasu v2: uses HasuCCData.FindMyCCAbilities for self (talent+spellbook detection),
-- and keeps existing party entries until ScanInspectTalents fills them in.
PopulateCCUsers = function()
    local seen = {}

    -- Self: use Hasu talent/spellbook detection
    ...
    -- We create a skeleton entry if none exists yet;
    -- ScanInspectTalents will fill in the real CC list once inspect data is available.
    for i = 1, 4 do
        local unit = "party" .. i
        if UnitExists(unit) then
            local name = UnitName(unit)
            ...
            if name and cls then
                seen[name] = true
                if not ccAddonUsers[name] then
                    -- Skeleton until inspect arrives
                    ccAddonUsers[name] = {
                        class         = cls,
                        specID        = 0,
                        hasAddon      = false,
...
-- ccInfo    : { name, baseCd, cdEnd, maxCharges, icon }
-- userInfo  : { class, ... }
-- rem       : remaining CD seconds
...
local function RenderCCBar(bar, playerName, ccInfo, userInfo, rem, isUnknown, chargeIdx, chargeMax)
    ...
    bar.nameText:SetText(playerName)
    bar.nameText:SetTextColor(unpack(FONT_COLOR))
    if bar.spellText then
        local spellLabel = ccInfo.name or "?"
        -- show per-charge indicator (k/N) when we are rendering one bar per charge
        if chargeIdx and chargeMax and chargeMax > 1 then
            spellLabel = spellLabel .. " |cFFAAAAAA(" .. chargeIdx .. "/" .. chargeMax .. ")|r"
        elseif (ccInfo.maxCharges or 1) > 1 then
            spellLabel = spellLabel .. " |cFFAAAAAA(x" .. (ccInfo.maxCharges) .. ")|r"
        end
        bar.spellText:SetText(spellLabel)
    end
    -- Details!-style fill: grows left→right as CD completes (full = READY)
    local baseCd = ccInfo.baseCd or 0
    local maxVal = math.max(1, baseCd)
...
-- Tick: update CC bars.
-- Hasu v2: one bar per (player, spellID) pair.
local function UpdateCCDisplay()
    if not ccFrame or not ccFrame:IsShown() then return end
    local _, barH = GetCCBarLayout()
    local now = GetTime()

    -- Build flat entry list: one row per CC ability per player
    local entries = {}
    for playerName, userInfo in pairs(ccAddonUsers) do
        if not (db.ccHiddenClasses and db.ccHiddenClasses[userInfo.class]) then
            local order = userInfo.ccOrder or {}
            -- If ccOrder is empty but ccs has entries (legacy/CCCAST-only), iterate ccs
            if #order == 0 then
                for sid in pairs(userInfo.ccs or {}) do table.insert(order, sid) end
            end
            for _, spellID in ipairs(order) do
                local cc = userInfo.ccs and userInfo.ccs[spellID]
                if cc then
                    -- Filter: interrupts hidden by default; respect per-spec ccSpellState
                    local isIntSpell = HASU_INTERRUPT_SPELL_SET[spellID]
                    local specSt  = db.ccSpellState and db.ccSpellState[userInfo.specID or 0]
                    local exState = specSt and specSt[spellID]
                    -- Interrupt: shown only if explicitly forced true
                    -- CC:        shown unless explicitly forced false
                    local doShow
                    if isIntSpell then
                        doShow = (exState == true)
                    else
                        doShow = (exState ~= false)
                    end
                    if doShow then
                        local rem = (cc.cdEnd and cc.cdEnd > now) and (cc.cdEnd - now) or 0
    ...
    -- Sort: ready entries first, then shortest CD; within same readiness sort by player name.
    -- For multi-charge spells, keep charge 1 before charge 2 when otherwise equal.
    table.sort(entries, function(a, b)
        local aReady = a.rem <= 0
        local bReady = b.rem <= 0
        if aReady ~= bReady then return aReady end
        if a.rem ~= b.rem then return a.rem < b.rem end
        local ak = (a.playerName or "") .. (a.cc.name or "")
        local bk = (b.playerName or "") .. (b.cc.name or "")
        if ak ~= bk then return ak < bk end
        return (a.chargeIdx or 0) < (b.chargeIdx or 0)
    end)
...
CreateCCFrame = function()
    ...
    ccFrame = CreateFrame("Frame", "HasuCCFrame", UIParent)
    ccFrame:SetWidth(fw)
...
-- Toggle CC Tracker from settings checkbox or /hasu cc
...
-- ============================================================
--  CC Spell Config window
--  /hasu cc — choose which CC spells to display per spec.
--
--  Interrupts are hidden by default (they appear in the
--  interrupt tracker window). CCs are visible by default.
--  State is saved in db.ccSpellState[specID][spellID]:
--    nil   → default   (CC=show / interrupt=hide)
--    true  → force show
--    false → force hide
-- ============================================================
local ccSpellConfigFrame = nil

local CC_SPEC_LIST = {
    {"Chev. mort",  "DEATHKNIGHT",{{250,"Sang"},{251,"Givre"},{252,"Impie"}}},
    {"Chass. démon","DEMONHUNTER",{{577,"Dévastat."},{581,"Vengeance"},{1480,"Dévoration"}}},
    ...
    {"Mage",        "MAGE",       {{62,"Arcane"},{63,"Feu"},{64,"Givre"}}},
    {"Moine",       "MONK",       {{268,"Brasseur"},{269,"M.vent"},{270,"T.brume"}}},
    {"Paladin",     "PALADIN",    {{65,"Sacré"},{66,"Protect."},{70,"Vindicte"}}},
    {"Prêtre",      "PRIEST",     {{256,"Discipline"},{257,"Sacré"},{258,"Ombre"}}},
    {"Voleur",      "ROGUE",      {{259,"Assassinat"},{260,"H.loi"},{261,"Finesse"}}},
    {"Chaman",      "SHAMAN",     {{262,"Élémentaire"},{263,"Améliorat."},{264,"Restaur."}}},
    {"Démoniste",   "WARLOCK",    {{265,"Affliction"},{266,"Démonologie"},{267,"Destructn."}}},
    {"Guerrier",    "WARRIOR",    {{71,"Armes"},{72,"Fureur"},{73,"Protect."}}},
}

ShowCCSpellConfig = function()
    if ccSpellConfigFrame then
        ccSpellConfigFrame:Show(); ccSpellConfigFrame:Raise(); return
    end
    ...
    local f = CreateFrame("Frame", "HasuCCSpellConfigFrame", UIParent, "BasicFrameTemplate")
    ...
    f:SetScript("OnDragStop",  f.StopMovingOrSizing)
    f:SetFrameStrata("DIALOG")
    table.insert(UISpecialFrames, "HasuCCSpellConfigFrame")

    -- Title
    local ttl = f:CreateFontString(nil,"OVERLAY")
    ttl:SetFont(FONT_FACE, 11, FONT_FLAGS)
    ttl:SetPoint("TOP", 0, -10)
    ttl:SetTextColor(0, 0.87, 0.87, 1)
    ttl:SetText("Hasu - Sorts CC a afficher")

    local selClass, selSpec = 1, 1

    -- ── CLASS BUTTONS (2 rows × 7) ────────────────────────────
    local cW = math.floor((W - PAD*2) / 7) - 1
    local classBtns = {}
    for i, cd in ipairs(CC_SPEC_LIST) do
        local col = CLASS_COLORS[cd[2]] or {0.8,0.8,0.8}
        local c   = (i-1) % 7
        local r   = math.floor((i-1) / 7)
        local btn = CreateFrame("Button", nil, f, "UIPanelButtonTemplate")
        btn:SetSize(cW, BTN_H)
        btn:SetPoint("TOPLEFT", f, "TOPLEFT",
            PAD + c*(cW+1), -(28 + r*(BTN_H+2)))
        btn:GetFontString():SetFont(FONT_FACE, 7, FONT_FLAGS)
        btn:GetFontString():SetTextColor(col[1],col[2],col[3],1)
        btn:SetText(cd[1])
        btn._i = i
        btn:SetScript("OnClick", function(self)
            selClass = self._i; selSpec = 1
            f:RefreshSpec(); f:RefreshSpells()
        end)
        classBtns[i] = btn
    end
    ...
    specArea:SetPoint("TOPLEFT", f,"TOPLEFT", PAD, -(28+2*(BTN_H+2)+4))
    specArea:SetSize(W-PAD*2, BTN_H)

    -- ── SEPARATOR ─────────────────────────────────────────────
    local sep = f:CreateTexture(nil,"ARTWORK")
    sep:SetColorTexture(0.3,0.3,0.3,0.9)
    sep:SetPoint("TOPLEFT", f,"TOPLEFT", PAD, -(28+2*(BTN_H+2)+4+BTN_H+6))
    sep:SetSize(W-PAD*2, 1)

    -- ── SCROLL LIST ───────────────────────────────────────────
    local listY = -(28+2*(BTN_H+2)+4+BTN_H+10)
    local listH = H + listY - 42

    local sf = CreateFrame("ScrollFrame","HasuCCSpcScroll",f,"UIPanelScrollFrameTemplate")
    sf:SetPoint("TOPLEFT", f,"TOPLEFT", PAD, listY)
    sf:SetSize(W-PAD*2-20, listH)

    local sc = CreateFrame("Frame", nil, sf)
    sc:SetSize(W-PAD*2-20, MAX_R*ROW_H)
    sf:SetScrollChild(sc)

    local rows = {}
    for i = 1, MAX_R do
        local row = CreateFrame("Frame", nil, sc)
        row:SetSize(W-PAD*2-20, ROW_H)
        row:SetPoint("TOPLEFT", sc,"TOPLEFT", 0, -(i-1)*ROW_H)

        local hl = row:CreateTexture(nil,"BACKGROUND")
        hl:SetAllPoints(); hl:SetColorTexture(1,1,1,0.04); hl:Hide()
        ...
        ic:SetSize(16,16); ic:SetPoint("LEFT",cb,"RIGHT",2,0)
        ic:SetTexCoord(0.07,0.93,0.07,0.93)
        row.icon = ic

        local nm = row:CreateFontString(nil,"OVERLAY")
        nm:SetFont(FONT_FACE,9,FONT_FLAGS)
        nm:SetPoint("LEFT",ic,"RIGHT",4,0); nm:SetWidth(136)
        nm:SetJustifyH("LEFT"); nm:SetWordWrap(false)
        row.name = nm

        local cd_fs = row:CreateFontString(nil,"OVERLAY")
        cd_fs:SetFont(FONT_FACE,9,FONT_FLAGS)
        cd_fs:SetPoint("LEFT",nm,"RIGHT",2,0); cd_fs:SetWidth(32)
        cd_fs:SetJustifyH("RIGHT"); cd_fs:SetTextColor(0.7,0.7,0.7,1)
        row.cdfs = cd_fs

        local tg = row:CreateFontString(nil,"OVERLAY")
        tg:SetFont(FONT_FACE,8,FONT_FLAGS)
        tg:SetPoint("LEFT",cd_fs,"RIGHT",4,0); tg:SetWidth(82)
        tg:SetJustifyH("LEFT"); tg:SetWordWrap(false)
        row.tag = tg
    ...
    -- ── BOTTOM BUTTONS ────────────────────────────────────────
    local rstBtn = CreateFrame("Button",nil,f,"UIPanelButtonTemplate")
    rstBtn:SetSize(120,22)
    rstBtn:SetPoint("BOTTOMLEFT",f,"BOTTOMLEFT",PAD,10)
    rstBtn:GetFontString():SetFont(FONT_FACE,9,FONT_FLAGS)
    rstBtn:SetText("Réinitialiser spec")

    local clsBtn = CreateFrame("Button",nil,f,"UIPanelButtonTemplate")
    clsBtn:SetSize(80,22)
    clsBtn:SetPoint("BOTTOMRIGHT",f,"BOTTOMRIGHT",-PAD,10)
    clsBtn:GetFontString():SetFont(FONT_FACE,9,FONT_FLAGS)
    clsBtn:SetText("Fermer")
    clsBtn:SetScript("OnClick", function() f:Hide() end)

    -- ── RefreshSpec ───────────────────────────────────────────
    function f:RefreshSpec()
        for _, sb in ipairs(specBtns) do sb:Hide() end
        wipe(specBtns)
        local specs = CC_SPEC_LIST[selClass][3]
        local sW = math.floor((W-PAD*2)/#specs) - 1
        for j, sp in ipairs(specs) do
            local sb = CreateFrame("Button",nil,specArea,"UIPanelButtonTemplate")
            sb:SetSize(sW, BTN_H)
            sb:SetPoint("TOPLEFT",specArea,"TOPLEFT",(j-1)*(sW+1),0)
            sb:GetFontString():SetFont(FONT_FACE,9,FONT_FLAGS)
            sb:SetText(sp[2])
            sb._j = j
            sb:SetScript("OnClick", function(self)
                selSpec = self._j; f:RefreshSpells()
            end)
            specBtns[j] = sb
        end
        if selSpec > #specs then selSpec = 1 end
    end

    -- ── RefreshSpells ─────────────────────────────────────────
    function f:RefreshSpells()
        local specID = CC_SPEC_LIST[selClass][3][selSpec][1]
        local ccList = HasuCCData and HasuCCData.SPEC_CC_DATA
            and HasuCCData.SPEC_CC_DATA[specID]

        if not db.ccSpellState then db.ccSpellState = {} end
        if not db.ccSpellState[specID] then db.ccSpellState[specID] = {} end
        local stMap = db.ccSpellState[specID]

        local function isIntSpell(sid)
            return HasuCCData and HasuCCData.INTERRUPT_SPELL_IDS
                and HasuCCData.INTERRUPT_SPELL_IDS[sid]
        end

        local ccE, intE = {}, {}
        if ccList then
            for _, e in ipairs(ccList) do
                if isIntSpell(e.spellID) then intE[#intE+1]=e
                else ccE[#ccE+1]=e end
            end
        end

        local ridx = 0
        local function AddRow(entry, interrupt)
            ridx = ridx + 1
            if ridx > MAX_R then return end
            local row = rows[ridx]
            local sid = entry.spellID

            -- Determine shown state
            local shown = interrupt and (stMap[sid]==true)
                       or (not interrupt and stMap[sid]~=false)

            row.check:Show()
            row.check:SetChecked(shown)
            row.check:SetScript("OnClick", function(self)
                if not db.ccSpellState then db.ccSpellState={} end
                if not db.ccSpellState[specID] then db.ccSpellState[specID]={} end
                local chk = self:GetChecked()
                if interrupt then
                    db.ccSpellState[specID][sid] = chk and true or nil
                else
            ...
            row.icon:SetTexture(ok_ic and tex or "Interface\\Icons\\INV_Misc_QuestionMark")

            local r_,g_,b_ = 0.9,0.9,0.9
            if interrupt then r_,g_,b_ = 0.5,0.5,0.5 end
            row.name:SetTextColor(r_,g_,b_,1)
            row.name:SetText(entry.name or "?")

            row.cdfs:SetText(
                (entry.baseCd and entry.baseCd>0) and (entry.baseCd.."s") or "—")

            local tagStr = ""
            if interrupt then
                tagStr = "|cFFFF6644[Interrupt]|r"
            elseif entry.requireTalent then
                tagStr = "|cFFFFDD00[Talent req.]|r"
            elseif entry.extraChargeTalent then
                tagStr = "|cFF44DDFF[\xc3\x972 talent]|r"
            end
            row.tag:SetText(tagStr)
            row:Show()
        end

        for _, e in ipairs(ccE) do AddRow(e, false) end

        if #intE > 0 and ridx < MAX_R then
            ridx = ridx + 1
            local row = rows[ridx]
            row.check:Hide()
            row.icon:SetTexture(nil)
            row.name:SetText("|cFF555555\xe2\x80\x94 Interrupts \xe2\x80\x94|r")
            row.name:SetTextColor(1,1,1,1)
            row.cdfs:SetText(""); row.tag:SetText("")
            row.check:SetScript("OnClick", nil)
            row:Show()
        end

        for _, e in ipairs(intE) do AddRow(e, true) end

        sc:SetHeight(math.max(listH, ridx*ROW_H+4))
        for i = ridx+1, MAX_R do
            rows[i]:Hide()
            if rows[i].check then rows[i].check:Show() end
        end
    end

    rstBtn:SetScript("OnClick", function()
        local specID = CC_SPEC_LIST[selClass][3][selSpec][1]
        if db.ccSpellState then db.ccSpellState[specID] = nil end
        ccDirty = true; f:RefreshSpells()
    end)

    -- Auto-jump to own class/spec on first open
    if myClass then
        for i, cd in ipairs(CC_SPEC_LIST) do
            if cd[2] == myClass then
                selClass = i
                local si = GetSpecialization and GetSpecialization()
                local cur = si and select(1, GetSpecializationInfo(si))
                if cur then
                    for j, sp in ipairs(cd[3]) do
                        if sp[1] == cur then selSpec=j; break end
                    end
                end
                break
            end
...
ShowCCConfig = function()
    ...
    ccConfigFrame = CreateFrame("Frame", "HasuCCConfigFrame", UIParent, "BasicFrameTemplate")
    ...
    local y = sy - 24
    -- Hasu v2: class checkboxes — no longer tied to a single primary CC
    local CLASS_DISPLAY_NAME = {
        WARRIOR     = "Warrior",     ROGUE       = "Rogue",
    ...
    for _, cls in ipairs(CC_CLASS_ORDER) do
        ...
        if lbl then
            ...
            lbl:SetText("|cFF" .. hex .. (CLASS_DISPLAY_NAME[cls] or cls) .. "|r")
        end
